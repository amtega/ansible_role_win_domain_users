...
Function PrincipalGroups {
  Param ($username, $args_extra)
  $retries = 0
  $failed = $true
  do {
    try{
      $groups = Get-ADPrincipalGroupMembership $username @args_extra -ErrorAction SilentlyContinue
      $failed = $false
    } catch {
      sleep 1
    }
  } while ($retries -lt 60 -and $failed)
  $result_groups = @()
  Foreach ($group in $groups) {
      $result_groups += $group.DistinguishedName
  }
...
If ($state -eq 'present') {
    ...
    try {
        ...
        If ($groups -ne $null) {
            ...
            $assigned_groups = PrincipalGroups $username $extra_args
...
try {
    If ($user_obj) {
        ...
        $result.groups = PrincipalGroups $username $extra_args
